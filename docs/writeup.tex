...
\item Built a fully local speech loop that records microphone audio,
    ...
    the cleaned reply through Kokoro or Kitten TTS running on Apple MLX.
\item The agent now logs timing for each major step (record, transcribe, LLM, TTS) to help profile latency and diagnose slowdowns.
\item TTS model selection (Kokoro/Kitten) and voice are controlled by environment or CLI; errors in model loading are now logged and surfaced.
